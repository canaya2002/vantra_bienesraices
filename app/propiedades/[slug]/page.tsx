...
// ... importaciones de componentes existentes (PropertyGallery, etc) ...
import PropertyGallery from '@/components/PropertyGallery';
import ContactForm from '@/components/ContactForm';
import PropertyFeatures from '@/components/PropertyFeatures';
import PropertyMap from '@/components/PropertyMap';
import { ArrowLeft, Bed, Bath, Square, Car, MapPin, Home, Calendar, MessageCircle } from 'lucide-react';

interface PropertyPageProps {
  params: Promise<{ slug: string }>;
}

// SSG: Generamos las rutas estáticas al momento del build para velocidad extrema
...
// SEO: Generación dinámica de metatags
export async function generateMetadata({ params }: PropertyPageProps): Promise<Metadata> {
  const resolvedParams = await params;
  const property = getPropertyBySlug(resolvedParams.slug);
  
  if (!property) {
    return { title: 'Propiedad no encontrada | Vantra' };
  }
...
export default async function PropertyPage({ params }: PropertyPageProps) {
  ...
  // 1. Schema de la Propiedad (Listing)
  const propertySchema = generatePropertySchema(property);
  
  // 2. Schema de Breadcrumbs (Navegación)
  ...
  const whatsappMessage = encodeURIComponent(
    `Hola Carlos, me interesa la propiedad "${property.title}" en ${property.neighborhood}. ID: ${property.id}.`
  );
  ...
  return (
    <>
      ...
      <article className="pt-24 pb-16">
        {/* ... Resto de tu código UI (Breadcrumb visual, Galería, Info, etc.) ... */}
        {/* MANTENER EL CÓDIGO VISUAL QUE YA TENÍAS, SOLO CAMBIÓ EL HEADER SEO ARRIBA */}
        ...
        <div className="container-custom">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
             {/* ... COPIAR EL RESTO DE TU JSX ORIGINAL AQUÍ (Main Content y Sidebar) ... */}
