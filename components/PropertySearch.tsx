...
const squareMetersRange = { min: 50, max: 2000 };

// --- COMPONENTE: SELECTOR PERSONALIZADO ---
const CustomSelect = ({ icon: Icon, value, options, onChange, placeholder = "Seleccionar" }) => {
  ...
  return (
    <div className="relative w-full lg:flex-1" ref={containerRef}>
      ...
      >
        ...
        {/* Corrección: Mostrar placeholder si value es 0 O cadena vacía */}
        <span className={`font-medium truncate ml-1 ${isSelected ? 'text-slate-800' : 'text-slate-500'}`}>
          {!value || value === 0 ? placeholder : value}
        </span>

        <ChevronDown 
          className={`absolute right-4 w-4 h-4 transition-transform duration-300 ${isOpen ? 'rotate-180 text-[#B8860B]' : 'text-gray-400'}`} 
        />
      </button>
...
// --- COMPONENTE: INPUTS DE RANGO ---
const RangeInputs = ({ min, max, value, onChange, prefix = "", suffix = "" }) => {
  return (
...
export default function PropertySearch({ onFilterChange }) {
  ...
  const hasActiveFilters = 
    ...
    filters.minPrice > 0 || filters.maxPrice > 0 || filters.minSquareMeters > 0;

  return (
    <section className={`sticky top-20 z-40 transition-all duration-500 ease-out px-4 ${isScrolled ? 'py-2' : 'py-6'}`}>
      <div className="container mx-auto max-w-6xl">
        ...
        >
          <div className="p-3 lg:p-4 flex flex-col lg:flex-row items-center gap-3 lg:gap-4 relative z-20">
            ...
            {/* 3. Habitaciones - Ancho aumentado para evitar cortes */}
            ...
            <div className="flex items-center gap-3 w-full lg:w-auto mt-2 lg:mt-0">
              ...
              >
                {/* FONDO DORADO METÁLICO (LUXURY GOLD) */}
                ...
                <div className="p-6 lg:p-8 grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-4">
                        <div className="p-2 bg-[#D4AF37]/10 rounded-lg text-[#B8860B]">
                    ...
                    <RangeInputs 
                        ...
                        onChange={(val) => {
                            handleFilterChange('minPrice', val.min);
                            handleFilterChange('maxPrice', val.max);
                        }}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <RangeInputs 
                        ...
                        onChange={(val) => {
                            handleFilterChange('minSquareMeters', val.min);
                            handleFilterChange('maxSquareMeters', val.max);
                        }}
                    />
                  </div>

                  {/* Footer Limpio Sin Línea Ni Espacio Enorme */}
                  <div className="md:col-span-2 flex justify-end items-center pt-2">
                    {hasActiveFilters && (
                      <button
                        onClick={resetFilters}
                        className="text-sm text-red-500 hover:text-red-700 font-bold flex items-center gap-1 transition-colors px-4 py-2 hover:bg-red-50 rounded-lg"
                      >
                        <X className="w-4 h-4" />
                        Limpiar Filtros
                      </button>
